...
model = joblib.load(BASE_DIR / 'model_rf_TelcoCustomers.pkl')
scaler = joblib.load(BASE_DIR / 'scaler_TelcoCustomers.pkl')
...
async def predict(data: TelecomSchema):
    ...
    return {'Churn': 'Скорее всего клиент уйдёт' if prediction == 1 else 'Скорее всего клиент останется'}
